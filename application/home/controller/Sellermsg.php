...
class Sellermsg extends BaseSeller {
    ...
    public function msg_setting() {
        ...
        // 店铺消息模板列表
        $smt_list = model('storemsgtpl')->getStoremsgtplList(array(), 'smt_code,smt_name,smt_message_switch,smt_message_forced,smt_short_switch,smt_short_forced,storemt_mail_switch,storemt_mail_forced');
        ...
        if (!empty($smt_list)) {
            foreach ($smt_list as $key => $val) {
                ...
                if ($val['storemt_mail_switch']) {
                    // 是否强制接收，强制接收必须开启
                    $smt_list[$key]['sms_mail_switch'] = $val['storemt_mail_forced'] ? 1 : intval($setting_list[$val['smt_code']]['sms_mail_switch']);
    ...
    public function edit_msg_setting() {
        ...
        // 店铺消息模板
        $smt_info = model('storemsgtpl')->getStoremsgtplInfo(array('smt_code' => $code), 'smt_code,smt_name,smt_message_switch,smt_message_forced,smt_short_switch,smt_short_forced,storemt_mail_switch,storemt_mail_forced');
        if (empty($smt_info)) {
            return false;
        }
    ...
    public function save_msg_setting()
    {
        ...
        // 保存
        $data = array();
        $data['smt_code'] = $smt_info['smt_code'];
        $data['store_id'] = session('store_id');
        // 验证站内信是否开启
        if ($smt_info['smt_message_switch']) {
            $data['sms_message_switch'] = $smt_info['smt_message_forced'] ? 1 : intval(input('post.message_forced'));
        } else {
            $data['sms_message_switch'] = 0;
        }
        // 验证短消息是否开启
        if ($smt_info['smt_short_switch']) {
            $data['sms_short_switch'] = $smt_info['smt_short_forced'] ? 1 : intval(input('post.short_forced'));
        } else {
            $data['sms_short_switch'] = 0;
        }
        $data['sms_short_number'] = input('post.short_number', '');
        // 验证邮件是否开启
        if ($smt_info['storemt_mail_switch']) {
            $data['sms_mail_switch'] = $smt_info['storemt_mail_forced'] ? 1 : intval(input('post.mail_forced'));
        } else {
            $data['sms_mail_switch'] = 0;
        }
        $data['sms_mail_number'] = input('post.mail_number', '');
        $conditiion = array();
        $conditiion['smt_code'] = $smt_info['smt_code'];
