...
class Index extends BaseMall {
    ...
    public function index() {
        ...
        //楼层数据
        $floor_block=array();
	$goodsclass_list = db('goods_class')->where('gc_parent_id', 0)->where('gc_show', 1)->order('gc_sort asc')->select();
    ...
    private function getIndexData()
    {
        ...
        if (empty($index_data)) {
            $index_data = array();
            $index_data['recommend_list'] = model('goods')->getGoodsOnlineList(array('goods_commend' => 1), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price,goods_price', 0, '',5,'goods_commonid');
            //限时折扣
            ...
            //获取第一文章分类的前三篇文章
            $index_data['index_articles'] = db('article')->where('ac.ac_code', 'notice')->where('a.article_show', 1)->alias('a')->field('a.article_id,a.article_url,a.article_title')->order('a.article_sort asc,a.article_time desc')->limit(3)->join('__ARTICLE_CLASS__ ac', 'a.ac_id=ac.ac_id')->select();
    ...
    private function getFloorList($cate_id) {
        ...
        if (empty($result)) {
            //获取此楼层下的所有分类
            $goods_class_list = db('goods_class')->where('gc_parent_id=' . $cate_id)->select();
            //获取每个分类下的商品
            $goods_list = array();
            ...
            $hot_goods_class_list = db('goods_class')->where('gc_parent_id=' . $cate_id)->order('gc_sort desc')->limit(5)->select();
    ...
    public function josn_class() {
        ...
        $goods_class = $goodsclass_model->getGoodsclassListByParentId(intval($_GET['gc_id']));
        $array = array();
    ...
    public function josn_flea_class() {
        ...
        $goods_class = $fleaclass_model->getFleaclassList(array('fleaclass_parent_id' => intval($_GET['gc_id'])));
        $array = array();
        if (is_array($goods_class) and count($goods_class) > 0) {
            foreach ($goods_class as $val) {
