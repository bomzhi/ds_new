...
class Date extends BaseCron {
    ...
    public function _creat_bill() {
        ...
        foreach ($store_ids as $store_id) {
            ...
            $store_info = db('store')->where('store_id', $store_id)->field('store_id,store_name,seller_name,is_own_shop,store_bill_time')->lock(true)->find();
            if ($store_info) {
                try {
                    $data = array();
                    ...
                    if (!empty($data)) {
                        ...
                        $data['ob_state'] = ($store_info['is_own_shop'] == 1) ? 4 : 1;
    ...
    private function _arrival_notice() {
        ...
        // 删除30天之前的记录
        $arrivalnotice_model->delArrivalNotice(array('arrivalnotice_addtime' => array('lt', $strat_time), 'arrivalnotice_type' => 1));
        ...
        for ($i = 0; $i <= $times; $i++) {
            ...
            // 清楚发送成功的数据
            $arrivalnotice_model->delArrivalNotice(array('goods_id' => array('in', $goodsid_array)));
        }
    }
