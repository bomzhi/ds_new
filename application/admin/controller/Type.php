...
class Type extends AdminControl {
    ...
    function getSpecRelated($type_id){
        //规格关联列表
        $type_model = model('type');
            $spec_related = $type_model->typeRelatedList('type_spec', array('type_id' => $type_id), 'sp_id');
            $sp_related = array();
    ...
    function getBrandRelated($type_id){
        //类型与品牌关联列表
        $type_model = model('type');
            $brand_related = $type_model->typeRelatedList('type_brand', array('type_id' => $type_id), 'brand_id');
            $b_related = array();
    ...
    function getSpec($class_id,$type_id=0){
        ...
        $in_gc_id='0';
        //分类是否存在
        if($class_id && db('goods_class')->where('gc_id',$class_id)->value('gc_id')){
            ...
            $condition = array();
            $condition['class_id'] = array('in',$in_gc_id);    
    ...
    function getBrand($class_id,$type_id=0){
        ...
        $brand_model = model('brand');
            $brand_list = $brand_model->getBrandPassedList(array('class_id'=>array('in',$in_gc_id)));
            $b_list = array();
            $b_related = array();
            if($type_id){
            ...
            if (is_array($brand_list) && !empty($brand_list)) {
                foreach ($brand_list as $k => $val) {
                    ...
                    $b_list[$val['class_id']]['name'] = $val['brand_class'];
                }
            }
            ksort($b_list);
            return $b_list;
