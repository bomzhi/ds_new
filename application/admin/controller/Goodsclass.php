...
class Goodsclass extends AdminControl {
        ...
        if (!request()->isPost()) {
            ...
            if (input('post.submit_type') == 'del') {
                if (is_array($_POST['tag_id']) && !empty($_POST['tag_id'])) {
                    //删除TAG
                    $classtag_model->delGoodsclasstagByIds(implode(',', $_POST['tag_id']));
    ...
    public function nav_edit() {
        ...
        if (request()->isPost()) {
            $update = array();
            $update['gc_id'] = $gc_id;
            $update['cn_alias'] = input('post.cn_alias');
            if (isset($_POST['class_id'])&&is_array($_POST['class_id'])) {
                $update['goodscn_classids'] = implode(',', $_POST['class_id']);
            }
            if (isset($_POST['brand_id'])&&is_array($_POST['brand_id'])) {
                $update['goodscn_brandids'] = implode(',', $_POST['brand_id']);
            }
            $update['cn_adv1_link'] = input('post.cn_adv1_link');
            $update['cn_adv2_link'] = input('post.cn_adv2_link');
            if (!empty($_FILES['pic']['name'])) {//上传图片
            ...
            if (is_array($brand_list) && !empty($brand_list)) {
                foreach ($brand_list as $k => $val) {
                    ...
                    $b_list[$val['class_id']]['name'] = $val['brand_class'] == '' ? lang('ds_default') : $val['brand_class'];
                }
            }
            ksort($b_list);
