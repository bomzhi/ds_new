...
class Dstaglib extends TagLib{
    ...
    public function tagAdv($tag, $content) {
        ...
        $str = '<?php ';
        $str .= '$ap_id =' . $ap_id . ';';
        $str .= '$ad_position = db("adv_position")->cache(3600)->column("ap_id,ap_name,ap_width,ap_height","ap_id");';
        $str .= '$result = db("adv")->where("ap_id=$ap_id  and adv_enabled = 1 and adv_startdate < ' . strtotime(date('Y-m-d H:00:00')) . ' and adv_enddate > ' . strtotime(date('Y-m-d H:00:00')) . ' ")->order("slide_sort desc")->cache(3600)->limit("' . $limit . '")->select();';
        $str .= '
if(!in_array($ap_id,array_keys($ad_position)) && $ap_id)
{
  db("adv_position")->insert(array(
         "ap_id"=>$ap_id,
