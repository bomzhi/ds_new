...
class Articleclass extends Model
{
    ...
    public function getArticleclassList($condition,$order='ac_parent_id asc,ac_sort asc,ac_id asc'){
        $result = db('article_class')->where($condition)->order($order)->select();
        return $result;
    }
    ...
    public function getOneArticleclass($id){
        if (intval($id) > 0){
            $value = intval($id);
            $result = db('article_class')->where(array('ac_id'=>$value))->find();
            return $result;
        }else {
            return false;
        }
    ...
    public function addArticleclass($data){
        $result = db('article_class')->insertGetId($data);
        return $result;
    }
    ...
    public function editArticleclass($data,$ac_id){
        $result =db('article_class')->where("ac_id",$ac_id)->update($data);
        return $result;
    }
    ...
    public function delArticleclass($id){
        return db('article_class')->where("ac_id = '". intval($id) ."'")->delete();
    }
