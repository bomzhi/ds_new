...
class Goodsclassnav extends Model {
    ...
    public function getGoodsclassnavInfoByGcId($gc_id) {
        return db('goods_class_nav')->where(array('gc_id' => $gc_id))->find();
    }
    ...
    public function addGoodsclassnav($data) {
        return db('goods_class_nav')->insert($data);
    }
    /**
     * 编辑存分类导航设置
     * @access public
    ...
    public function editGoodsclassnav($update, $gc_id) {
        return db('goods_class_nav')->where(array('gc_id' => $gc_id))->update($update);
    }
}
?>
