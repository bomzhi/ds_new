...
class Goodsclasstag extends Model {
    ...
    public function delGoodsclasstag($condition) {
        return db('goods_class_tag')->where($condition)->delete();
    }
    ...
    public function getGoodsclasstagList($condition = array(), $page = '', $field = '*') {
        if ($page) {
            $result = db('goods_class_tag')->field($field)->where($condition)->paginate($page,false,['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
            $result = db('goods_class_tag')->field($field)->where($condition)->select();
            return $result;
        }
    }
    ...
    public function editGoodsclasstag($param,$gctag_id) {
        return db('goods_class_tag')->where("gctag_id = '" . $gctag_id . "'")->update($param);
    }
    ...
    public function delGoodsclasstagByIds($id) {
        return db('goods_class_tag')->where('gctag_id in (' . $id . ')')->delete();
    }
