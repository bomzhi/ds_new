...
class Goodsclass extends Model
{
    ...
    {
        $result = db('goods_class')->field($field)->where($condition)->order('gc_parent_id asc,gc_sort asc,gc_id asc')->limit(false)->select();
        return $result;
    }
    ...
    public function editGoodsclass($data = array(), $condition = array())
    {
        // 删除缓存
        $this->dropCache();
        return db('goods_class')->where($condition)->update($data);
    }
    ...
    public function delGoodsclass($condition)
    {
        // 删除缓存
        $this->dropCache();
        return db('goods_class')->where($condition)->delete();
    }
    ...
    public function get_type_brands($type_ids = array())
    {
        ...
        if (is_array($type_ids) && !empty($type_ids)) {
            ...
            $type_list = db('type_brand')->where(array('type_id' => array('in', $type_ids)))->limit(10000)->select();
    ...
    private function _getGoodsclassnavById($gc_id)
    {
        ...
        if (!empty($nav_info['cn_pic']) && file_exists($pic_name)) {
        ...
        if (!empty($nav_info['cn_adv1']) && file_exists($pic_name)) {
        ...
        if (!empty($nav_info['cn_adv2']) && file_exists($pic_name)) {
        ...
        if ($nav_info['cn_brandids'] != '') {
            $nav_info['cn_brands'] = $brand_model->getBrandList(array('brand_id' => array('in', $nav_info['cn_brandids'])));
    ...
    public function addGoodsclass($data)
    {
        // 删除缓存
        $this->dropCache();
        return db('goods_class')->insertGetId($data);
    }
