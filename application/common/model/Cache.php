...
class Cache extends Model {
    ...
    private function _goodsclassseo() {
        ...
        $list = db('goods_class')->field('gc_id,gc_title,gc_keywords,gc_description')->where(array('gc_keywords' => array('neq', '')))->limit(false)->select();
        if (!is_array($list))
            return null;
        $array = array();
        foreach ($list as $k => $v) {
    ...
    private function _express() {
        $fields = 'express_id,express_name,express_code,express_letter,express_order,express_url,express_zt_state';
    ...
    private function _classtag() {
        ...
        $list = db('goods_class_tag')->field($field)->limit(false)->select();
        if (!is_array($list))
            return null;
        return $list;
    }
