...
class Storemsg extends Model
{
    ...
    public function addStoremsg($data) {
        ...
        $storemsg_id = db('store_msg')->insertGetId($data);
        if (config('node_chat')) {
    ...
    public function editStoremsg($condition, $update) {
        return db('store_msg')->where($condition)->update($update);
    }
    ...
    public function getStoremsgInfo($condition, $field = '*') {
        return db('store_msg')->field($field)->where($condition)->find();
    ...
    public function getStoremsgList($condition, $field = '*', $page = '0', $order = 'storemsg_id desc') {
        $return =db('store_msg')->field($field)->where($condition)->order($order)->paginate($page,false,['query' => request()->param()]);
    ...
    public function getStoremsgCount($condition) {
        return db('store_msg')->where($condition)->count();
    }
    ...
    public function delStoremsg($condition) {
        db('store_msg')->where($condition)->delete();
    }
}