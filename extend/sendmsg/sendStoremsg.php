...
class sendStoremsg {
    ...
    public function send($param = array()) {
        ...
        $setting_info = model('storemsgsetting')->getStoremsgsettingInfo(array('smt_code' => $this->code, 'store_id' => $this->store_id));
        // 发送站内信
        if ($tpl_info['smt_message_switch'] && ($tpl_info['smt_message_forced'] || $setting_info['sms_message_switch'])) {
            $message = ds_replace_text($tpl_info['smt_message_content'],$param);
        ...
        // 发送邮件
        if ($tpl_info['storemt_mail_switch'] && $setting_info['sms_mail_number'] != '' && ($tpl_info['storemt_mail_forced'] || $setting_info['sms_mail_switch'])) {
            $param['site_name'] = config('site_name');
